...
from pantos.servicenode.plugins.restapi_plugin import PLUGINInteractor
...
class _MonitorStatus(flask_restful.Resource):
    """RESTful resource for Service Node Status

    """
    def get(self) -> flask.Response:
        # monitor_status = []
        # monitor_status.append({
        #             'TEST1': 1,
        #             'TEST2': int(2)})
        try:
            monitor_status = PLUGINInteractor().PLUGIN_get_fee_sums()
        except Exception:
            _logger.critical('unable to process Monitor Status request', exc_info=True)
            internal_server_error()

        return ok_response(monitor_status)
...
_restful_api.add_resource(_MonitorStatus, '/health/status')                 # Extension